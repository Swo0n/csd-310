...
for purchase_cost in resultP:
    print("Total Purchase Sales: $" + "".join(map(str,purchase_cost)))
...
for rental_cost in resultR:
    print("Total Rental Sales: $" + "".join(map(str,rental_cost)))
...
cursor3 = mydb.cursor()
cursor3.execute("SELECT inventory.product_name, equipment.restock_date from inventory JOIN equipment ON inventory.inventory_id = equipment.inventory_id WHERE equipment.restock_date < DATE_SUB('2023-07-15', INTERVAL 5 YEAR)")

print("-- DISPLAYING Expired Equip RECORDS --")
result3 = cursor3.fetchall()

for product_name, restock_date in result3:
